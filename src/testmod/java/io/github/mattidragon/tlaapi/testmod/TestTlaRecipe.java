...
public record TestTlaRecipe(TestRecipe entry) implements TlaRecipe {
    @Override
    ...
    public List<TlaIngredient> getInputs() {
        return List.of(TlaIngredient.ofIngredient(entry.input()));
    }

    @Override
    public List<TlaStack> getOutputs() {
        return List.of(TlaStack.of(entry.output()));
    }
    ...
    public void buildGui(GuiBuilder builder) {
        ...
        builder.addSlot(TlaIngredient.ofIngredient(entry.input()), 0, 4).addTooltip(Text.literal("hi")).markInput();
        ...
        builder.addSlot(TlaStack.of(entry.output()).withChance(0.5), 46, 0).markOutput().makeLarge().addTooltip(Text.literal("am large"));
